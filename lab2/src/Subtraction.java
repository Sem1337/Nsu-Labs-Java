...
public class Subtraction implements Command {
    ...
    public Subtraction(String args) {
        ...
        if(!args.isEmpty())throw new WrongNumberOfArguments("wrong number of arguments in command Subtraction.\nexpected 0. found " + cnt);
    }

    @Override
    public void execute(DataStorage data) {
        if(data.getOperands().size() < 2)throw new NotEnoughOperands("not enough operands in stack for Subtraction command: expected 2. found " + data.getOperands().size());
